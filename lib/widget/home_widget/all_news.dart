...
import 'package:news_app_v2/bloc/get_all_news_bloc.dart';
import 'package:news_app_v2/element/error_element.dart';
import 'package:news_app_v2/element/loader.dart';
import 'package:news_app_v2/model/article.dart';
import 'package:news_app_v2/model/article_response.dart';
import 'package:news_app_v2/screen/news_detail.dart';
import 'package:news_app_v2/style/theme.dart' as Style;
import 'package:timeago/timeago.dart' as timeago;

class AllNewsWidget extends StatefulWidget {
  @override
  _AllNewsWidgetState createState() => _AllNewsWidgetState();
}
...
class _AllNewsWidgetState extends State<AllNewsWidget> {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<ArticleResponse>(
      stream: getAllNewsBloc.subject.stream,
      builder: (context, AsyncSnapshot<ArticleResponse> snapshot) {
        if (snapshot.hasData) {
          if (snapshot.data.error != null && snapshot.data.error.length > 0) {
            return buildErrorWidget(snapshot.data.error);
          }
          return _buildAllNewsWidget(snapshot.data);
  ...
  Widget _buildAllNewsWidget(ArticleResponse data) {
